...
export default class ProjectFetcher
{
    ...
    private logProjectFiles(): void
    {
        ...
        {
            footage += ('require.resolve("../../_media/images/' + this.projects[i].thumbnail + '"); \n');
    ...
    {
        let prePath: string = './src/_media/images/projects/'
        this.projects.push(
            LifeLike.getProject(prePath + 'LifeLike/'),
            ExpositionWebsite.getProject(prePath + 'ExpositionWebsite/'),
            Mythos.getProject(prePath + 'Mythos/'),
            CONFINED_SPACE.getProject(prePath + 'CONFINED_SPACE/'),
            PersonalSharedPhysical.getProject(prePath + 'PersonalSharedPhysical/'),
            MovingUp.getProject(prePath + 'MovingUp/'),
            FallingFood.getProject(prePath + 'FallingFood/'),
        );
    }
    ...
    private loadDailies(logProjectsData: boolean = false): void
    {
        ...
        for (let i: number = 0; i < dailiesKeys.length; i++)
        {
            ...
            {
                videos += "require.resolve('./src/_media/images/dailies/" + splitURL[4] + ".mp4');"
                thumbnails += "require.resolve('./src/_media/images/dailies/thumbnails/" + splitURL[4] + ".jpg');"
            }

            daily.footage = ['./src/_media/images/dailies/' + splitURL[4] + '.mp4'];
            daily.thumbnail = './src/_media/images/dailies/thumbnails/' + splitURL[4] + '.jpg';
    ...
    {
        if (tags[0] == 'All') { console.log('returning all'); return this.projects; }
