...
var $author$project$Types$GotNewScreenSize = F2(
	function (a, b) {
...
var $author$project$Types$TimeTick = function (a) {
	return {$: 'TimeTick', a: a};
};
...
var $author$project$Main$animator = A4(
	$mdgriffith$elm_animator$Animator$watchingWith,
	function ($) {
		return $.hovered;
	},
	F2(
		function (newHovered, model) {
			return _Utils_update(
				model,
				{hovered: newHovered});
		}),
	...
	A4(
		...
		F2(
			function (newFootageAbout, model) {
				return _Utils_update(
					model,
					{footageAbout: newFootageAbout});
			}),
		function (_v4) {
			return false;
		},
		A4(
			$mdgriffith$elm_animator$Animator$watchingWith,
			function ($) {
				return $.footageTransition;
			},
			F2(
				function (newTransition, model) {
					return _Utils_update(
						model,
						{footageTransition: newTransition});
				}),
			function (_v3) {
				return false;
			},
			A4(
				...
				F2(
					function (newTransition, model) {
						return _Utils_update(
							model,
							{projectTransition: newTransition});
					}),
				function (_v2) {
					return false;
				},
				A4(
					$mdgriffith$elm_animator$Animator$watchingWith,
					function ($) {
						return $.activeViewerPart;
					},
					F2(
						function (newViewPart, model) {
							return _Utils_update(
								model,
								{activeViewerPart: newViewPart});
						}),
					function (_v1) {
						return false;
					},
					A4(
						...
						F2(
							function (newLoadedStatus, model) {
								return _Utils_update(
									model,
...
var $author$project$Types$Description = {$: 'Description'};
var $author$project$Project$Final = {$: 'Final'};
var $author$project$Types$PageLoaded = function (a) {
	return {$: 'PageLoaded', a: a};
};
...
var $author$project$Project$Custom = function (a) {
	return {$: 'Custom', a: a};
};
var $justinmimbs$date$Date$Days = {$: 'Days'};
var $author$project$Project$Human = {$: 'Human'};
var $author$project$Project$Image = function (a) {
	return {$: 'Image', a: a};
};
var $author$project$Project$None = {$: 'None'};
...
var $author$project$Projects$DavidZwitser$data = function () {
	var age = A3(
		$justinmimbs$date$Date$diff,
		$justinmimbs$date$Date$Years,
		A3($justinmimbs$date$Date$fromCalendarDate, 1998, $elm$time$Time$Oct, 8),
		A3($justinmimbs$date$Date$fromCalendarDate, 2025, $elm$time$Time$Oct, 17));
	return {
		id: 'about',
		sources: {
			externalLink: $elm$core$Maybe$Nothing,
			finalFootage: _List_fromArray(
				[
					$author$project$Project$Image(
					{description: 'David Zwitser', fileName: 'in_code.jpeg'}),
					$author$project$Project$Custom(
					_List_fromArray(
						[
							{href: 'https://www.instagram.com/coelepinda/', textDescription: 'Instagram: @Coelepinda'},
							{href: 'https://www.youtube.com/@davidzwitser/videos', textDescription: 'Youtube: David Zwitser'},
							{href: 'mailto:talk@davidzwitser.com', textDescription: 'Email: talk@davidzwitser.com'},
							{href: 'https://maps.app.goo.gl/fSWAYYDFYP343BUU9', textDescription: 'Studio adress: Oudegracht 371 3511 PG'}
						])),
					$author$project$Project$Image(
					{description: 'On the road', fileName: 'pf.jpg'}),
					$author$project$Project$Image(
					{description: 'At the HKU exposition talking about work', fileName: 'interview.jpg'})
				]),
			folderName: 'DavidZwitser',
			processFootage: _List_Nil,
			thumbnail: {description: '', fileName: 'in_code.jpeg'}
		},
		tags: {medium: $author$project$Project$Human, toolStack: _List_Nil},
		text: {
			context: 'I was born in Haarlem in the Netherlands and am ' + ($elm$core$String$fromInt(age) + ' years old. I now live in Utrecht where I have a studio at de Oudegracht together with 6 other wonderfull makers. We also sometimes try to be a collective and work together under the name `Wildgroei`.'),
			interestingDetails: $elm$core$Maybe$Nothing,
		...
		variables: {
			client: $author$project$Project$None,
			clientLink: $elm$core$Maybe$Nothing,
			color: A3($mdgriffith$elm_ui$Element$rgb255, 82, 75, 65),
			date: A3($justinmimbs$date$Date$fromCalendarDate, 1998, $elm$time$Time$Oct, 8),
			hoursSpent: A3(
				$justinmimbs$date$Date$diff,
				$justinmimbs$date$Date$Days,
				A3($justinmimbs$date$Date$fromCalendarDate, 1998, $elm$time$Time$Oct, 8),
				A3($justinmimbs$date$Date$fromCalendarDate, 2025, $elm$time$Time$Oct, 17)),
			teamMates: $elm$core$Maybe$Nothing,
			teamSize: 1
		}
	};
}();
var $author$project$Projects$AllProjects$defaultProject = $author$project$Projects$DavidZwitser$data;
...
var $author$project$Project$TechnicalExploration = {$: 'TechnicalExploration'};
var $author$project$Project$Video = function (a) {
	return {$: 'Video', a: a};
};
var $author$project$Project$YoutubeEmbedded = function (a) {
	return {$: 'YoutubeEmbedded', a: a};
};
var $mdgriffith$elm_ui$Element$rgb = F3(
	function (r, g, b) {
		return A4($mdgriffith$elm_ui$Internal$Model$Rgba, r, g, b, 1);
	});
var $author$project$Projects$BQN$data = {
	id: 'BQN',
	sources: {
		externalLink: $elm$core$Maybe$Just('https://github.com/DavidZwitser/BQN-Experiments'),
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Image(
				{description: 'A short view of what the language looks like (it\'s the top line)', fileName: 'language2.jpg'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'A live coding tool experiment I worked on in BQN with my own custom editor using an inmplementation of raylib in BQN (Rayed-BQN)', fileName: 'crz6kn71RXY?si=rlrkvI5d3pWIfd4l'}),
				$author$project$Project$Video(
				{description: 'A fun particle system which throws around BQN characters, also in Rayed-BQN', fileName: 'Particles.mov'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'Writing snake in 8 minutes in BQN', fileName: 'tOZde7zrsLM?si=_BpcA8dnz4yQYi64'}),
				$author$project$Project$Image(
				{description: 'A series of youtube videos on my channel about this language, me working in it, giving tutorials and my ideas around it!', fileName: 'youtube.png'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'Me being allowed to talk on my favoroute poscast which is about languages like this', fileName: 'nrSst9keXO8?si=k1KgPkV1OlEteqKT'}),
				$author$project$Project$Image(
				{description: 'The promotion for a workshop I gave about this language', fileName: 'workshop_adverticement.png'}),
				$author$project$Project$Image(
				{description: 'An image of that workshop!', fileName: 'workshop_image.jpg'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'A livestream looking at the code behind the text editor for the live-coding tool and making some more progress', fileName: '57oi4hBBlj0?si=zvy5WZU-Le0zR4zu'}),
				$author$project$Project$Video(
				{description: 'Another video of that LiveCoding tool. I like the LFO interactive system I made to quickly and intuitively add movements', fileName: 'LiveCoder2.mov'}),
				$author$project$Project$Video(
				{description: 'A astroids replica I made in Rayed-BQN', fileName: 'astroids.mov'}),
				$author$project$Project$Video(
				{description: 'A snake replica I made in Rayed-BQN', fileName: 'snake.mov'}),
				$author$project$Project$Image(
				{description: 'An extension I made for the ZED code editor to run BQN. Here I learned about LSP\'s and TreeSitter!', fileName: 'language_extension_zed.png'})
			]),
		folderName: 'BQN',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'language2.jpg'}
	},
	tags: {medium: $author$project$Project$TechnicalExploration, toolStack: _List_Nil},
	text: {context: '', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'I found BQN, a very unique programming language with a lot of body and history behind it, which taught me to a lot of new ways to think about programming and algorithms. It inspired me to do a whole bunch of things which are viewable in this image galery.', myRole: '', name: 'BQN explorations', philosophy: '', processDescription: '', reflection: '', shortDescription: 'A bunch of things I did around this programming language I fell in love with.'},
	variables: {
		client: $author$project$Project$None,
		clientLink: $elm$core$Maybe$Nothing,
		color: A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2023, $elm$time$Time$Jul, 1),
		hoursSpent: 0,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 0
	}
};
var $author$project$Project$Audio = function (a) {
	return {$: 'Audio', a: a};
};
var $author$project$Project$HKU = {$: 'HKU'};
var $author$project$Project$Installation = {$: 'Installation'};
var $author$project$Project$Pneumatics = {$: 'Pneumatics'};
var $author$project$Project$PureData = {$: 'PureData'};
var $author$project$Project$RaspberryPI = {$: 'RaspberryPI'};
var $author$project$Project$Woodwork = {$: 'Woodwork'};
var $author$project$Projects$BuildUpAndRelease$data = {
	...
	sources: {
		...
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Video(
				{description: 'The installation at HKU exposure', fileName: 'OmDeHoek.mov'}),
				$author$project$Project$Video(
				{description: 'A close-up of the UI', fileName: 'DeUI.mov'}),
				$author$project$Project$Image(
				{description: 'People using the machine', fileName: 'InHetHokje.jpg'}),
				$author$project$Project$Video(
				{description: 'A bit deconstructed', fileName: 'Aankleding.mov'}),
				$author$project$Project$Video(
				{description: 'The release moment', fileName: 'Losgelaten.mov'})
			]),
		folderName: 'BuildUpAndRelease',
		processFootage: _List_fromArray(
			[
				$author$project$Project$Image(
				{description: 'Building plans I created in Fusion 360', fileName: 'project_scetches.jpg'}),
				$author$project$Project$Audio(
				{description: 'A snipped of the ideas that were going trough my head during the creation process', fileName: 'philosophy_behind_it.wav'}),
				$author$project$Project$Image(
				{description: 'Some small sketches giving an idea of what the installation could look like', fileName: 'small_sketches.jpg'}),
				$author$project$Project$Video(
				{description: 'An early prototype used by a teacher during class', fileName: 'rene_tryout.mov'}),
				$author$project$Project$Video(
				{description: 'A later prototype trying out different visuals', fileName: 'test.mov'}),
				$author$project$Project$Image(
				{description: 'A prototype version', fileName: 'Ben_tryout.PNG'}),
				$author$project$Project$Video(
				{description: 'Behind the prototype version', fileName: 'behind_the_prototype.mov'}),
				$author$project$Project$Image(
				{description: 'A view of working on the internals of the machine', fileName: 'cluttered_proress.jpg'}),
				$author$project$Project$Video(
				{description: 'A view of constructing the case of the machine', fileName: 'glueing_the_case.mov'}),
				$author$project$Project$Video(
				{description: 'Moving the machine around', fileName: 'moving_with_bakfiets.mov'}),
				$author$project$Project$Image(
				{description: 'Final touches', fileName: 'Thuis.jpg'})
			]),
		thumbnail: {description: '', fileName: 'usage.jpg'}
	},
	tags: {
		medium: $author$project$Project$Installation,
		toolStack: _List_fromArray(
			[$author$project$Project$RaspberryPI, $author$project$Project$Woodwork, $author$project$Project$Pneumatics, $author$project$Project$PureData])
	},
	text: {context: 'This was my graduation artwork at Image and Media Technology at the HKU.', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'After a long reflection on what drives frustrates and motivates David Zwitser, he managed to compile it to a concrete interaction. The resulting  machine is addictive, triggering and captivating. It grabs you and keep you there up untill you collect the will to forcefully seperate yourself from it. When you press its luring big red button, a minimal alarming loading bar with a percentage next to it will pop up on the screen. The noise the machine is making will slowly get louder and higher the further this loading bar travels. First it goes quickly, but the longer you stand there, holding that button, the slower it goes. At this point you have gotten this far, so why would you stop, you want to see what will happen when the loading bar is full don\'t you? You hold on, the bar crawls up every so slightly and you wait. Who knows, it might still reach 100%. Don\'t be foolish, keep on going.', myRole: '', name: 'Build Up and Release', philosophy: '', processDescription: 'This machine comes from a pattern of operation I recognice within myself. It took a while to grasp and understand it, and eventually trough the labour of the creative process I found an interaction which represents it.', reflection: '', shortDescription: 'Experience the pointlessness and frustration behind our constant desire to want and accumulate.'},
	variables: {
		client: $author$project$Project$HKU,
		clientLink: $elm$core$Maybe$Nothing,
		color: A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2022, $elm$time$Time$Jun, 30),
		hoursSpent: 1500,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 1
	}
};
var $author$project$Project$Arduino = {$: 'Arduino'};
var $author$project$Project$Building = {$: 'Building'};
var $author$project$Project$Cinedans = {$: 'Cinedans'};
var $author$project$Projects$CONFINED_SPACE$data = {
	id: 'confined_space',
	sources: {
		externalLink: $elm$core$Maybe$Nothing,
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Image(
				{description: '', fileName: 'Intensiteit_van_de_ervaring.jpg'}),
				$author$project$Project$YoutubeEmbedded(
				{description: '', fileName: 'vSZDguIEcEE'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Holy_tube.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Edge.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Full_standalone.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Inside_tha_tube.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Holo.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Tube.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Deep_dive.jpg'})
			]),
		folderName: 'CONFINED_SPACE',
		processFootage: _List_fromArray(
			[
				$author$project$Project$Video(
				{description: 'trying out what it feels like to dance with something inanimate as part of our creative process', fileName: 'dancing_with_machine.mov'}),
				$author$project$Project$Image(
				{description: 'using some sandwich bags to see how easy it is to make an inflatable (it is not)', fileName: 'broodzakjes_prototype.jpg'}),
				$author$project$Project$Video(
				{description: 'experimenting with what it feels like to be compressed with a air sucking foot pump and\ngarbage bag', fileName: 'vacuum_yourself_experiment.mov'}),
				$author$project$Project$Image(
				{description: 'blowing up a very big bag and experiencing what it is to be inside it', fileName: 'inside_big_bag.jpg'}),
				$author$project$Project$Video(
				{description: 'using a way bigger bag', fileName: 'room_filling_plastics.mov'}),
				$author$project$Project$Video(
				{description: 'making the bag breath up and down', fileName: 'pressure_release.mov'}),
				$author$project$Project$Image(
				{description: 'overview of process till then for a presentation', fileName: 'green_light_presentation.jpg'}),
				$author$project$Project$Video(
				{description: 'another mockup of the experience, this time between two tables to create more pressure', fileName: 'compressed.mov'}),
				$author$project$Project$Image(
				{description: 'first prototype using TPU material', fileName: 'TPU_prototype.jpg'}),
				$author$project$Project$Image(
				{description: 'bringing fresh wood to the wood-workshop to build the frame', fileName: 'bringing_wood_to_workshop.jpg'}),
				$author$project$Project$Video(
				{description: 'sawing the angles in the pillars', fileName: 'sawing_angle.mov'}),
				$author$project$Project$Image(
				{description: 'sitting zen in the newly constructed frame', fileName: 'zen_in_frame.jpg'}),
				$author$project$Project$Video(
				{description: 'a miniature version of the materials we want to use', fileName: 'holographic_tpu_experiment.mov'}),
				$author$project$Project$Video(
				{description: 'using plastic bag in newly build frame', fileName: 'Derk_vacuumed_in_frame_first_time.mov'}),
				$author$project$Project$Image(
				{description: 'me in the frame for the first time', fileName: 'inside_frame_prototype.jpg'})
			]),
		thumbnail: {description: '', fileName: 'Tube.jpg'}
	},
	tags: {
		medium: $author$project$Project$Installation,
		toolStack: _List_fromArray(
			[$author$project$Project$Arduino, $author$project$Project$Woodwork, $author$project$Project$Building])
	},
	text: {context: 'This project was a school assignment where we collaborated with the Cinedance festival with the goal of it being displayed at an actual festival in the Eye Museum. We were selected for the festival, but sadly corona spoiled it.', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'A 2x2x2m hexagonal wooden frame with an inflatable cushion made of TPU constrained within it. Step inside and feel how you are slowly being pressurised and released. Experience how the see-trough inside allows you to see the holographic belly of the cushion. The experience is unique and strangely comfortable.', myRole: 'The brainstorming and testing was a group effort. The construction was divided between the frame and cushion. Me and Derk designed and build the frame, and Joia and Brit designed and sowed the cushion.', name: 'CONFINED SPACE', philosophy: 'This pre corona installation plays with spaces inside spaces. It hides you from the rest of the world and uses all your senses to try and calm you down and give you a safe space to be in. It plays in to our world becoming increasingly more complex and intimidating due to technology endlessly enhancing our possibilities. Thus we present another technological contraption to solve this problem.', processDescription: 'This project had a long and thorough process. We researched a ton of different possibilities and the work presented a lot of different new techniques to learn.', reflection: 'I learned thinking in analogue material and inflatable structures. I also increased my ability to think in tactile experiences. The outcome is a very interesting, out of my general comfort-zone piece which I absolutely loved to work.', shortDescription: 'An intimate tactile experience where you step inside of an artificial bubble which tries to calm and sooth you.'},
	variables: {
		client: $author$project$Project$Cinedans,
		clientLink: $elm$core$Maybe$Just('https://cinedans.nl'),
		color: A3($mdgriffith$elm_ui$Element$rgb255, 115, 116, 117),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2020, $elm$time$Time$Jan, 15),
		hoursSpent: 320,
		teamMates: $elm$core$Maybe$Just(
			_List_fromArray(
				[
					_Utils_Tuple2('Derk Roelofsen', 'https://www.instagram.com/flerk_lab/'),
					_Utils_Tuple2('Joia van de Ven', 'https://joiavandeven.myportfolio.com/confined-space'),
					_Utils_Tuple2('Britt Vendel', 'https://brittvendel.myportfolio.com/confined-space')
				])),
		teamSize: 4
	}
};
var $author$project$Project$AudioVisual = {$: 'AudioVisual'};
var $author$project$Projects$CanWorld$data = {
	id: 'can_world',
	sources: {
		externalLink: $elm$core$Maybe$Nothing,
		finalFootage: _List_fromArray(
			[
				$author$project$Project$YoutubeEmbedded(
				{description: '', fileName: 'eSeoGAsGSIQ'}),
				$author$project$Project$Image(
				{description: '', fileName: 'smeer.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'starting_noise.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'floating_paper.jpg'})
			]),
		folderName: 'CanWorld',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'noisy_noise.jpg'}
	},
	tags: {medium: $author$project$Project$AudioVisual, toolStack: _List_Nil},
	text: {context: '', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'This was a collaboration project between me and a conservatory student named Can Yursten. This was my first time working together with a musician and I am quite glad with how it audio and visuals go together.', myRole: '', name: 'Can world', philosophy: '', processDescription: '', reflection: '', shortDescription: 'A audio visual experience made together with Can Yursten'},
	variables: {
		client: $author$project$Project$None,
		clientLink: $elm$core$Maybe$Nothing,
		color: A3($mdgriffith$elm_ui$Element$rgb255, 151, 142, 120),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2020, $elm$time$Time$Sep, 15),
		hoursSpent: 24,
		teamMates: $elm$core$Maybe$Just(
			_List_fromArray(
				[
					_Utils_Tuple2('Can Yursteven', 'https://canyurtseven.com')
				])),
		teamSize: 2
	}
};
var $author$project$Project$DataAnalysis = {$: 'DataAnalysis'};
var $author$project$Project$GOLANG = {$: 'GOLANG'};
var $author$project$Project$Python = {$: 'Python'};
var $author$project$Project$SETUP = {$: 'SETUP'};
var $author$project$Project$Touchdesigner = {$: 'Touchdesigner'};
var $author$project$Project$VimeoEmbedded = function (a) {
	return {$: 'VimeoEmbedded', a: a};
};
var $author$project$Projects$CuddleKing2000$data = {
	id: 'cuddle_king_2000',
	sources: {
		externalLink: $elm$core$Maybe$Nothing,
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Image(
				{description: 'Someone using the Cuddle King during an exposition in Hoog Catherijne', fileName: 'HUG.jpg'}),
				$author$project$Project$Image(
				{description: 'A view of the receipt that is printed for you after using the machine', fileName: 'bonnetje.jpg'}),
				$author$project$Project$Image(
				{description: 'Someone using the Cuddle King as seen form above', fileName: 'top_down_hug.jpg'}),
				$author$project$Project$Image(
				{description: 'A look at the TouchDesigner network', fileName: 'CODE.png'}),
				$author$project$Project$Video(
				{description: 'A view of the spot where we presented the Cuddle King 2000 in Hoog Catherijne', fileName: 'hearth_view_hoog_catherijne.mov'}),
				$author$project$Project$Image(
				{description: 'An over the shoulder view of someone using the Cuddle King', fileName: 'over_shoulder_picture.jpg'}),
				$author$project$Project$VimeoEmbedded(
				{description: 'A video showing the exposition in Hoog Catherijne', fileName: 'https://player.vimeo.com/video/794884983?h=0f62f716df'})
			]),
		folderName: 'CuddleKing2000',
		processFootage: _List_fromArray(
			[
				$author$project$Project$YoutubeEmbedded(
				{description: 'A video showing a bit of the creative process', fileName: 'ATCA8wG33Y4?si=gK2dqwoaY1CN6Yjt'}),
				$author$project$Project$Image(
				{description: 'a prototype version with the torso', fileName: 'prototype.jpg'}),
				$author$project$Project$Image(
				{description: 'Juliette Mout trying out different cloths to dress the torso', fileName: 'cloth_tryout.jpg'}),
				$author$project$Project$Video(
				{description: 'A snapshot of the process of making the real time hug simulation arms', fileName: 'hug_hands_moving.mov'}),
				$author$project$Project$Image(
				{description: 'A view of some notes I made during the creative process', fileName: 'notitites.jpg'}),
				$author$project$Project$Video(
				{description: 'Digitally reconstructing a hug to get to understand how the movements go', fileName: 'reconstructing_hug.mov'}),
				$author$project$Project$Video(
				{description: 'A first stage of the torso with sensors on it', fileName: 'torso_wired_up_tryout.mov'}),
				$author$project$Project$Video(
				{description: 'An experiment about making eye contact during a date', fileName: 'oogcontact_experiment.mov'}),
				$author$project$Project$Video(
				{description: 'Experimenting with the effect of distance and movement during a date', fileName: 'afstand_experiment.mov'}),
				$author$project$Project$Image(
				{description: 'Data collected from interviewing pub owners in Utrecht', fileName: 'barmannen_data.jpg'})
			]),
		thumbnail: {description: '', fileName: 'HUG.jpg'}
	},
	tags: {
		medium: $author$project$Project$Installation,
		toolStack: _List_fromArray(
			[$author$project$Project$Touchdesigner, $author$project$Project$Python, $author$project$Project$Arduino, $author$project$Project$GOLANG, $author$project$Project$DataAnalysis])
	},
	text: {context: 'I made this installation during my internship at SETUP Utrecht.', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'Based on the question if social friction in our modern world is outdated and should be fully optimized with technology, we tried to optimise one of the most awkward interactions there is. Dating. We set our focus on the first impression: the greeting hug. This machine sports a huggable torso suspended with ropes in a metal frame. When you hug it, a whole bunch of sensors will start collecting data about what you are doing. It wil calculate patterns from it and compare those to the hugs given to it earlier. As a result you will get feedback on how your first date hug compares to the optimum. It\'ll show you if you hugged for too long or not, where your hands were on the back and where they should have been and how much the pressure you exerted deviates from the optimal pressure you should give. We presented this machine at the mall in Utrecht (Hoog Catherijne), posing as a real startup company trying to get this into the market. Through this we wanted to see how people would react to something as extreme as quantifying and optimising something as subjective and emotional as a hug. Does the market still want to retain the human softness and uncertainty in our interactions or are we ready for the full digitising of everything or do we and love. If we must beleive our Hoog Catherijne visitors, it will be the latter.', myRole: '', name: 'De Cuddle King 2000', philosophy: '', processDescription: 'This project has a rich creative process. From interviewing a whole bunch of pub owners to researching about dating coaches. Throughout I made a whole lot of tests and experiments and trough this we got to our final product.', reflection: '', shortDescription: 'Awkwardness is outdated. Optimize your dating life by training to give the ultimate first date greeting hug!'},
	variables: {
		client: $author$project$Project$SETUP,
		clientLink: $elm$core$Maybe$Just('https://www.setup.nl/projecten/cuddle-king-2000/'),
		color: A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2023, $elm$time$Time$Feb, 1),
		hoursSpent: 1500,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 2
	}
};
var $author$project$Project$Tellart = {$: 'Tellart'};
var $author$project$Projects$Dinner2050$data = {
	id: 'Dinner2050',
	sources: {
		externalLink: $elm$core$Maybe$Nothing,
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Image(
				{description: 'A nice picture of the installation with me in the front', fileName: 'dinner-in-2050-your-dish.jpg'}),
				$author$project$Project$Image(
				{description: 'The nice wooden structure the installation was in', fileName: 'dinner-in-2050.jpg'}),
				$author$project$Project$VimeoEmbedded(
				{description: 'A video of the user experience of the installation', fileName: 'https://player.vimeo.com/video/933313136'}),
				$author$project$Project$Image(
				{description: 'The animation stack system (sadly I cant show a picture of the StateCircuits implementation)', fileName: 'animation_stack.png'})
			]),
		folderName: 'Dinner2050',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'dinner-in-2050-your-dish.jpg'}
	},
	tags: {medium: $author$project$Project$Installation, toolStack: _List_Nil},
	text: {context: '', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'I got to assist as a TouchDesigner developer on this interactive installation at the climate conference COP28 in Dubai in 2023. I worked on the GLSL based animations and the overal system. I got to implement my StateCircuits in an actual application and designed a modular animation COMP sequencer. It was a very valuable and educational experience.', myRole: '', name: 'Dinner 2050', philosophy: '', processDescription: 'I got to mostly help on-site in Dubai, implementing and optimizing the system. We had seperate designers and producers, which allowed me to totally focus on technical execution and implementation.', reflection: '', shortDescription: 'An interactive installation at COP28 in Dubai'},
	variables: {
		client: $author$project$Project$Tellart,
		clientLink: $elm$core$Maybe$Just('https://www.tellart.com/projects/dinner-in-2050-at-cop28/'),
		color: A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2023, $elm$time$Time$Dec, 1),
		hoursSpent: 0,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 0
	}
};
var $author$project$Project$Networking = {$: 'Networking'};
var $author$project$Project$ProjectionMapping = {$: 'ProjectionMapping'};
var $author$project$Projects$LifeLike$data = {
	id: 'life_like',
	sources: {
		externalLink: $elm$core$Maybe$Nothing,
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Image(
				{description: 'People interacting with the piece', fileName: 'Life_action.jpg'}),
				$author$project$Project$Video(
				{description: 'A video of someone walking around it', fileName: 'in_action.mov'}),
				$author$project$Project$Image(
				{description: 'The sensors on the top (which I should have actually build a casing for in retrospect)', fileName: 'Sensors.jpg'}),
				$author$project$Project$Image(
				{description: 'Another picture of the sensors', fileName: 'Sensors_top.jpg'}),
				$author$project$Project$Image(
				{description: 'The projection from the front', fileName: 'Front_projection.jpg'}),
				$author$project$Project$Image(
				{description: 'A pictuer of the surrounding reserach', fileName: 'Process_behind.jpg'})
			]),
		folderName: 'LifeLike',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'Front_projection.jpg'}
	},
	tags: {
		medium: $author$project$Project$Installation,
		toolStack: _List_fromArray(
			[$author$project$Project$Touchdesigner, $author$project$Project$RaspberryPI, $author$project$Project$ProjectionMapping, $author$project$Project$Networking])
	},
	text: {
		context: 'This was the final test of my propaeduitic year.',
		interestingDetails: $elm$core$Maybe$Just('The result is a physical multi media installation which has a unique effect on the viewer.'),
	...
	variables: {
		client: $author$project$Project$HKU,
		clientLink: $elm$core$Maybe$Just('https://www.hku.nl/studeren-aan-hku/media/image-and-media-technology'),
		color: A3($mdgriffith$elm_ui$Element$rgb255, 29, 100, 65),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2019, $elm$time$Time$Jun, 10),
		hoursSpent: 320,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 1
	}
};
var $author$project$Projects$PersonalSharedPhysicsl$data = {
	...
	sources: {
		...
		finalFootage: _List_fromArray(
			[
				$author$project$Project$YoutubeEmbedded(
				{description: '', fileName: 'oseUdJMYaFk'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Personal_smalltree.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Personal.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Physical_3d.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Physical_chop.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Physical_dat.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Physical_pixels.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'Shared.jpg'})
			]),
		folderName: 'PersonalSharedPhysical',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'thumb_texture.jpg'}
	},
	tags: {medium: $author$project$Project$Installation, toolStack: _List_Nil},
	text: {context: 'This project was part of an exposition I did the first half a year of my education at the HKU.', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'An installation based on a theory about our world being separated into three different realms: our Personal internal world, our Shared communal world and the Physical nearly unquantifiable world. It consisted of a single dial which you can rotate to the left or right, to scroll trough an attempt to visualise these different realities. The personal one shows a full grown tree with colors offset from those of reality and my own music playing; the shared one is an as neutral view of the world and the tree as possible, making it follow the rules we expect and all cohere to; and finally the physical one is represented by the data behind the tree, going into more and more abstract versions of it, finally ending in a single pixel of data.', myRole: '', name: 'Personal Shared Physical', philosophy: '', processDescription: '', reflection: '', shortDescription: 'Exploring a theory of three different separated realities.'},
	variables: {
		client: $author$project$Project$None,
		clientLink: $elm$core$Maybe$Nothing,
		color: A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2019, $elm$time$Time$Jan, 19),
		hoursSpent: 40,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 1
	}
};
var $author$project$Projects$PledgeStation$data = {
	id: 'pledge_station',
	sources: {
		externalLink: $elm$core$Maybe$Nothing,
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Image(
				{description: 'A great image made by the Tellart team showing someone in front of the final installation', fileName: 'use.jpg'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'An interactive previs tool I made for experimenting during the pre-implementation stage', fileName: '0MrpMJxeoPc?si=BPl77dfzQk8nu6O3'}),
				$author$project$Project$Image(
				{description: 'An image of us projecting the visual in the prototype phase', fileName: 'prototype.jpg'}),
				$author$project$Project$Image(
				{description: 'The Pledge Station network halfway during development (it got more structured, well named and minimal after)', fileName: 'pledge_network.png'}),
				$author$project$Project$Image(
				{description: 'A picture of the Dutch Pavillion in the World Expo', fileName: 'building.jpg'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'The trailer for the Dutch pavillion on Tellart\'s youbtube channel', fileName: '2TucCktwmKg?si=T-ah4ktQeF0KgvUL'}),
				$author$project$Project$Image(
				{description: 'A picture of the Phillipines Pavillion at World Expo', fileName: 'PhillipinesBuilding.jpg'}),
				$author$project$Project$Image(
				{description: 'The introduction show space where I helped', fileName: 'Woven-Introduction-1-©️-Tellart.jpeg'}),
				$author$project$Project$Video(
				{description: 'A torn down/ renamed example of what the editor system for the introduction show looked like. It was also designed to share render power between computers', fileName: 'editor_system.mov'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'The trailer for the Philippine pavillion on Tellart\'s youbtube channel', fileName: 'JTwsfW7KoKo?si=0AnAm2kkhW0qfXaE'})
			]),
		folderName: 'PledgeStation',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'use.jpg'}
	},
	tags: {medium: $author$project$Project$Installation, toolStack: _List_Nil},
	text: {
		...
		interestingDetails: $elm$core$Maybe$Just('The pledge station previs originally used raymarched reflections I hand coded which simulated the infinity mirror effect more faithfully. I removed it since it was more performant to hard code simulate the infinity mirror effect. It was very educational to program tho!'),
		longDescription: 'I had the pleasure of working together with Tellart, where I got to help out with two pavillions. One was the Dutch pavillion where I got to develop the visual/ interactive part of an infinity mirror installation designed to leave the visitors with optimism and inpsiration. The other was the introduction projection show for the Philippines pavillion where I got to develop tools and setup a system for that show. A product of that was a nice timeline editor, render and playout tool in TouchDesigner originally also designed to distribute real time render power between two pc\'s.',
		myRole: '',
	...
	variables: {
		client: $author$project$Project$Tellart,
		clientLink: $elm$core$Maybe$Just('https://www.tellart.com/projects/netherlands-pavilion-at-world-expo-2025/'),
		color: A3($mdgriffith$elm_ui$Element$rgb, 0.8, 0.75, 0.9),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2025, $elm$time$Time$Apr, 4),
		hoursSpent: 0,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 0
	}
};
var $author$project$Projects$StateCircuits$data = {
	id: 'StateCircuits',
	sources: {
		externalLink: $elm$core$Maybe$Just('https://olib.amb-service.net/component/state-circuit-snippets'),
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Video(
				{description: 'A little promotion video I made for the tool in reel form', fileName: 'teaser_reel.mov'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'A game and all the network blocks that make it work usnig StateCircuits!', fileName: 'GavNvm8BLts?si=UgzH0e94hUUm8dn8'}),
				$author$project$Project$Video(
				{description: 'Binding analogue button sounds to the network movements', fileName: 'interact.mov'}),
				$author$project$Project$Video(
				{description: 'The same but with a timer', fileName: 'timer.mov'}),
				$author$project$Project$Video(
				{description: 'Some more example snippets', fileName: 'example_systems.mov'}),
				$author$project$Project$YoutubeEmbedded(
				{description: 'An explainer video for the tool', fileName: 'oC1592EncG4?si=YblP9K_T1a__615-'})
			]),
		folderName: 'StateCircuits',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'thumbnail.png'}
	},
	tags: {medium: $author$project$Project$TechnicalExploration, toolStack: _List_Nil},
	text: {context: '', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'A tool I developed to handle state in TouchDesigner', myRole: '', name: 'StateCircuits', philosophy: '', processDescription: '', reflection: '', shortDescription: ''},
	variables: {
		client: $author$project$Project$None,
		clientLink: $elm$core$Maybe$Nothing,
		color: A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2023, $elm$time$Time$Jun, 2),
		hoursSpent: 0,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 0
	}
};
var $author$project$Project$EXboot = {$: 'EXboot'};
var $author$project$Projects$WhooshWhoosh$data = {
	id: 'whoosh_whoosh',
	sources: {
		externalLink: $elm$core$Maybe$Just(''),
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Image(
				{description: '', fileName: 'DAVIDZ-Laag-2.jpg'}),
				$author$project$Project$YoutubeEmbedded(
				{description: '', fileName: 'jrPWUyBuM24?si=Ljp9fdDgrKscecWg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'DAVIDZ-Laag-1.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'DAVIDZ-Laag-3.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'DAVIDZ-Laag-5.jpg'}),
				$author$project$Project$Image(
				{description: '', fileName: 'DAVIDZ-Laag-6.jpg'})
			]),
		folderName: 'WhooshWhoosh',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'DAVIDZ-Laag-2.jpg'}
	},
	tags: {medium: $author$project$Project$Installation, toolStack: _List_Nil},
	text: {context: 'Whoosh Whoosh is an artwork that comes from a love for programming.', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'In contrast to works of its kind, it steps back from being purely an experience and shows its own code openly. It is a celebration of us humans having created this digital machinery and the power it gives us to create. With a minimal amount of nodes, a lot is going on. Besides this it is also an exploration of the relationship between the work, the software and the maker. Each tool is a canvas for the maker to draw on, and it is a shame to hide that. Lastly, for its maker, it is a conscious step back from social issues or philosophical reflection. That is why its name is purposefully non descriptive. Letting go of the endless reflecting and analysing, making something for the beauty of making.', myRole: '', name: 'Whoosh Whoosh', philosophy: '', processDescription: '', reflection: '', shortDescription: ''},
	variables: {
		client: $author$project$Project$EXboot,
		clientLink: $elm$core$Maybe$Just('https://exutrecht.nl/en/exhibitions/woosh-woosh'),
		color: A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2024, $elm$time$Time$May, 1),
		hoursSpent: 160,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 1
	}
};
var $author$project$Projects$WorkInProgress$data = {
	id: 'WIP',
	sources: {
		externalLink: $elm$core$Maybe$Nothing,
		finalFootage: _List_fromArray(
			[
				$author$project$Project$Video(
				{description: '(WIP) An exploration of UX/UI (systems we design for our physical bodies to enter the digital world) as a purely esthetic device (made in TouchDesigner).', fileName: 'crane.mov'}),
				$author$project$Project$Image(
				{description: '(WIP) An exploration of UX/UI (systems we design for our physical bodies to enter the digital world) as a purely esthetic device (made in TouchDesigner).', fileName: 'crane.png'}),
				$author$project$Project$Image(
				{description: '(WIP) An exploration of UX/UI (systems we design for our physical bodies to enter the digital world) as a purely esthetic device (made in TouchDesigner).', fileName: 'crane2.png'}),
				$author$project$Project$Video(
				{description: '(WIP) A small tool for building chords. (Turn on audio with the speaker icon on the bottom left). I\'ll add a lot more features like chord inverstions/ voicings, a real time piano roll, a sequencer, audio filters and the likes. This was initially build to hook up to the experiment before this one. I realised that after learning music theory I could put that theory into a TD tool.', fileName: 'harmony.mov'}),
				$author$project$Project$Video(
				{description: '(WIP) After VJing for the first time and slapping down a scrappy but cool TD tool/UI for it, I got inspired to build a propper one. It\'s called `MixMachine` with the goal that you can drag components around to form a pipeline of little mixer components. Everything should feel intuitive and it is designed to map to finite MIDI controll inputs where you can drag your MIDI Chop channel onto a UI element where it will be auto mapped, and that mapping would be tweakable in the side menu. A bit ambitious.', fileName: 'MixMachine.mov'}),
				$author$project$Project$Video(
				{description: '(WIP) Another part of the design process of this MixMachine. (It\'s just fun to make playfull UI\'s like this).', fileName: 'MixMachineUI.mov'}),
				$author$project$Project$YoutubeEmbedded(
				{description: '(WIP) I\'m learning to make music with Tydal Cycles, but  I used Strudl before. Live coding is such a wonderfull interface for making music.', fileName: 'v8h79bucNh4'}),
				$author$project$Project$Video(
				{description: '(WIP) Another short video with Strudel', fileName: 'strudl.mov'}),
				$author$project$Project$Video(
				{description: 'A small little UI world for my Buisness Card.', fileName: 'kaartje.mov'}),
				$author$project$Project$Video(
				{description: '(WIP) Audio experiment with very minimal TD using Bitwig', fileName: 'TD_Bitwig2.mov'}),
				$author$project$Project$Video(
				{description: '(WIP) Another audio experiment with very minimal TD using Bitwig', fileName: 'TD_Bitwig.mov'}),
				$author$project$Project$Image(
				{description: 'An iPhone shortcut makes a screenshot but also takes a photo of the place your phone was in in that moment', fileName: 'spacial_pin.png'})
			]),
		folderName: 'WorkInProgress',
		processFootage: _List_Nil,
		thumbnail: {description: '', fileName: 'crane.png'}
	},
	tags: {medium: $author$project$Project$TechnicalExploration, toolStack: _List_Nil},
	text: {context: '', interestingDetails: $elm$core$Maybe$Nothing, longDescription: 'A series of experiments I\'m working on or have done (last update 20 Oct 2025). Read the descriptions on the bottom to learn more. If it says (WIP) then I still want to progress it. These are here because they don\'t warrant a full project.', myRole: '', name: '(WIP)Experiments', philosophy: '', processDescription: '', reflection: '', shortDescription: ''},
	variables: {
		client: $author$project$Project$None,
		clientLink: $elm$core$Maybe$Nothing,
		color: A3($mdgriffith$elm_ui$Element$rgb, 1, 1, 1),
		date: A3($justinmimbs$date$Date$fromCalendarDate, 2020, $elm$time$Time$Jan, 1),
		hoursSpent: 0,
		teamMates: $elm$core$Maybe$Nothing,
		teamSize: 0
	}
};
var $author$project$Projects$AllProjects$projects = _List_fromArray(
	[$author$project$Projects$DavidZwitser$data, $author$project$Projects$PledgeStation$data, $author$project$Projects$CuddleKing2000$data, $author$project$Projects$Dinner2050$data, $author$project$Projects$WorkInProgress$data, $author$project$Projects$StateCircuits$data, $author$project$Projects$BQN$data, $author$project$Projects$WhooshWhoosh$data, $author$project$Projects$BuildUpAndRelease$data, $author$project$Projects$CONFINED_SPACE$data, $author$project$Projects$LifeLike$data, $author$project$Projects$PersonalSharedPhysicsl$data, $author$project$Projects$CanWorld$data]);
var $elm$core$Basics$round = _Basics_round;
var $author$project$Main$init = F3(
	function (flags, url, key) {
		return _Utils_Tuple2(
			{
				activeViewerPart: $mdgriffith$elm_animator$Animator$init($author$project$Types$Description),
				allProjects: $author$project$Projects$AllProjects$projects,
				footageAbout: $mdgriffith$elm_animator$Animator$init($author$project$Project$Final),
				footageAutoplay: true,
				footageMuted: true,
				footageTransition: $mdgriffith$elm_animator$Animator$init(0),
				hovered: $mdgriffith$elm_animator$Animator$init($author$project$Types$Background),
				key: key,
				loaded: $mdgriffith$elm_animator$Animator$init(false),
				onMobile: false,
				projectTransition: $mdgriffith$elm_animator$Animator$init($author$project$Projects$AllProjects$defaultProject),
				screenSize: {h: 0, w: 0},
				url: url
			},
			$elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
						A2(
						$elm$core$Task$perform,
						$author$project$Types$PageLoaded,
						$elm$core$Task$succeed(true)),
						A2(
						$elm$core$Task$perform,
						$author$project$Types$UrlChanged,
...
var $author$project$Project$getAppropriateFootage = F2(
	function (about, project) {
		return _Utils_eq(about, $author$project$Project$Final) ? project.sources.finalFootage : project.sources.processFootage;
	});
...
var $author$project$Routing$DeadEnd = {$: 'DeadEnd'};
...
var $author$project$Routing$routeToProject = function (route) {
	if (route.$ === 'Project') {
		var name = route.a;
		return A2(
			$elm$core$Maybe$withDefault,
			$author$project$Projects$AllProjects$defaultProject,
			$elm$core$List$head(
				A2(
					$elm$core$List$filter,
					function (project) {
						return _Utils_eq(project.id, name);
					},
					$author$project$Projects$AllProjects$projects)));
	} else {
		return $author$project$Projects$AllProjects$defaultProject;
	}
};
var $author$project$Routing$Project = function (a) {
	return {$: 'Project', a: a};
};
...
var $author$project$Routing$urlParser = A2($elm$url$Url$Parser$map, $author$project$Routing$Project, $elm$url$Url$Parser$string);
var $author$project$Routing$urlToProject = function (url) {
	var fragUrl = _Utils_update(
		url,
		{
			fragment: $elm$core$Maybe$Nothing,
			path: A2($elm$core$Maybe$withDefault, '', url.fragment)
		});
	return $author$project$Routing$routeToProject(
		A2(
			$elm$core$Maybe$withDefault,
			$author$project$Routing$DeadEnd,
			A2($elm$url$Url$Parser$parse, $author$project$Routing$urlParser, fragUrl)));
};
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'PageLoaded':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							loaded: A3($mdgriffith$elm_animator$Animator$go, $mdgriffith$elm_animator$Animator$verySlowly, true, model.loaded)
						}),
					$elm$core$Platform$Cmd$none);
			case 'UrlChanged':
				var url = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							footageAbout: A3($mdgriffith$elm_animator$Animator$go, $mdgriffith$elm_animator$Animator$immediately, $author$project$Project$Final, model.footageAbout),
							footageTransition: A3($mdgriffith$elm_animator$Animator$go, $mdgriffith$elm_animator$Animator$slowly, 0, model.footageTransition),
							projectTransition: A3(
								$mdgriffith$elm_animator$Animator$go,
								$mdgriffith$elm_animator$Animator$slowly,
								$author$project$Routing$urlToProject(url),
			...
			case 'NextFootageClicked':
				var dir = msg.a;
				var currProjectImagesAmount = $elm$core$List$length(
					A2(
						$author$project$Project$getAppropriateFootage,
						$mdgriffith$elm_animator$Animator$current(model.footageAbout),
						$mdgriffith$elm_animator$Animator$current(model.projectTransition)));
				var currImageIndex = $mdgriffith$elm_animator$Animator$current(model.footageTransition);
				var newImageIndex = A3(
					$elm$core$Basics$clamp,
					0,
					currProjectImagesAmount,
					_Utils_eq(dir, $author$project$Types$Right) ? (currImageIndex + 1) : (currImageIndex - 1));
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							footageTransition: A3($mdgriffith$elm_animator$Animator$go, $mdgriffith$elm_animator$Animator$quickly, newImageIndex, model.footageTransition)
						}),
					$elm$core$Platform$Cmd$none);
			case 'FootageIndexClicked':
				var index = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							footageTransition: A3($mdgriffith$elm_animator$Animator$go, $mdgriffith$elm_animator$Animator$quickly, index, model.footageTransition)
						}),
					$elm$core$Platform$Cmd$none);
			case 'NewPagePartActive':
				var viewerPart = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							activeViewerPart: A2(
								...
								_List_fromArray(
									[
										A2($mdgriffith$elm_animator$Animator$event, $mdgriffith$elm_animator$Animator$slowly, viewerPart)
									]),
			...
			case 'NewHover':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							hovered: A3($mdgriffith$elm_animator$Animator$go, $mdgriffith$elm_animator$Animator$quickly, viewerPart, model.hovered)
						}),
					$elm$core$Platform$Cmd$none);
			case 'NewFootageTypeClicked':
				var newImageType = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							footageAbout: A3($mdgriffith$elm_animator$Animator$go, $mdgriffith$elm_animator$Animator$quickly, newImageType, model.footageAbout),
							footageTransition: A3($mdgriffith$elm_animator$Animator$go, $mdgriffith$elm_animator$Animator$immediately, 0, model.footageTransition)
						}),
					$elm$core$Platform$Cmd$none);
			case 'ToggleMute':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{footageMuted: !model.footageMuted}),
					$elm$core$Platform$Cmd$none);
			case 'ToggleAutoplay':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{footageAutoplay: !model.footageAutoplay}),
					$elm$core$Platform$Cmd$none);
			default:
				var value = msg.a;
				return _Utils_Tuple2(
...
var $author$project$Effects$LoadAnimation$loadFadein = function (pageLoaded) {
	return $mdgriffith$elm_animator$Animator$arrived(pageLoaded) ? $mdgriffith$elm_ui$Element$none : A2(
		...
		_List_fromArray(
			[
				...
				A2(
					...
					pageLoaded,
					function (loaded) {
						return loaded ? $mdgriffith$elm_animator$Animator$at(0) : $mdgriffith$elm_animator$Animator$at(1);
					}))
			]),
		$mdgriffith$elm_ui$Element$none);
};
var $author$project$Types$NewHover = function (a) {
	return {$: 'NewHover', a: a};
};
var $author$project$Types$NewPagePartActive = function (a) {
	return {$: 'NewPagePartActive', a: a};
};
var $author$project$Types$ProjectPicker = {$: 'ProjectPicker'};
var $author$project$Funcs$when = F3(
	function (condition, trueValue, falseValue) {
		return condition ? trueValue : falseValue;
	});
var $author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation = F4(
	function (model, viewPart, from, to) {
		return A2(
			$mdgriffith$elm_animator$Animator$move,
			model.activeViewerPart,
			function (state) {
				return A3(
					$author$project$Funcs$when,
					_Utils_eq(state, viewPart),
					$mdgriffith$elm_animator$Animator$at(to),
					$mdgriffith$elm_animator$Animator$at(from));
			});
	});
...
var $author$project$Types$FootageIndexClicked = function (a) {
	return {$: 'FootageIndexClicked', a: a};
};
var $author$project$Types$Left = {$: 'Left'};
var $author$project$Types$ToggleMute = {$: 'ToggleMute'};
...
var $author$project$Types$NextFootageClicked = function (a) {
	return {$: 'NextFootageClicked', a: a};
};
...
var $author$project$Viewers$ProjectsViewer$CenterView$CenterView$transitionAnimation = F4(
	function (transition, condition, from, to) {
		return A2(
			$mdgriffith$elm_animator$Animator$move,
			transition,
			function (state) {
				return condition(state) ? $mdgriffith$elm_animator$Animator$at(to) : $mdgriffith$elm_animator$Animator$at(from);
			});
	});
var $author$project$Viewers$ProjectsViewer$CenterView$CenterView$footageNavigationButtons = F5(
	function (dir, onDesktop, clickable, footageTransition, str) {
		return A2(
			$mdgriffith$elm_ui$Element$Input$button,
			_Utils_ap(
				onDesktop ? _List_fromArray(
					[
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$alpha(
						A4(
							$author$project$Viewers$ProjectsViewer$CenterView$CenterView$transitionAnimation,
							footageTransition,
							function (_v0) {
								return clickable;
							},
						...
						_Utils_eq(dir, $author$project$Types$Left) ? $mdgriffith$elm_ui$Element$Font$alignLeft : $mdgriffith$elm_ui$Element$Font$alignRight,
					...
					])),
			{
				...
				onPress: A3(
					...
					clickable,
					$elm$core$Maybe$Just(
						$author$project$Types$NextFootageClicked(dir)),
					$elm$core$Maybe$Nothing)
			});
	});
...
var $author$project$Project$getFootagePath = F2(
	function (footage, project) {
		return '../media/projects/' + (project.sources.folderName + ('/' + footage.fileName));
	});
...
var $bellroy$elm_embed_youtube$Embed$Youtube$Internal$Attribute$ModestBranding = {$: 'ModestBranding'};
var $bellroy$elm_embed_youtube$Embed$Youtube$Attributes$modestBranding = $bellroy$elm_embed_youtube$Embed$Youtube$Internal$Attribute$ModestBranding;
...
var $author$project$Viewers$ProjectsViewer$CenterView$FootageViewer$footageView = F4(
	function (projectTransition, footageTransition, muted, possibleFootage) {
		var project = $mdgriffith$elm_animator$Animator$current(projectTransition);
		var footageWrapper = function (footageEl) {
			return A2(
				$mdgriffith$elm_ui$Element$el,
				_List_fromArray(
					[
						...
						A2(
							...
							function (currProject) {
								return $mdgriffith$elm_animator$Animator$at(
									A3(
										$author$project$Funcs$when,
										_Utils_eq(project.id, currProject.id),
										1,
										0));
							})),
						$mdgriffith$elm_ui$Element$inFront(footageEl)
					]),
				$mdgriffith$elm_ui$Element$none);
		};
		var footageIndex = $mdgriffith$elm_animator$Animator$current(footageTransition);
		var footage = A2(
			$elm$core$Maybe$withDefault,
			$author$project$Project$Custom(
				_List_fromArray(
					[
						{href: '', textDescription: 'Footage not found'}
					])),
			possibleFootage);
		return footageWrapper(
			function () {
				switch (footage.$) {
					case 'Image':
						var data = footage.a;
											...
											A2(
												...
												_List_fromArray(
													[
														...
														$elm$html$Html$Attributes$id(data.fileName),
							...
							$mdgriffith$elm_ui$Element$html(
								$bellroy$elm_embed_youtube$Embed$Youtube$toHtml(
									A2(
										...
										_List_fromArray(
											[
												$bellroy$elm_embed_youtube$Embed$Youtube$Attributes$height(600),
												$bellroy$elm_embed_youtube$Embed$Youtube$Attributes$modestBranding
											]),
										$bellroy$elm_embed_youtube$Embed$Youtube$fromString(data.fileName)))));
					case 'VimeoEmbedded':
						var data = footage.a;
							...
							A2(
								...
								_List_fromArray(
									[
										...
										A2($elm$html$Html$Attributes$style, 'height', '100%'),
										A2($elm$html$Html$Attributes$style, 'object-fit', 'contain'),
										$elm$html$Html$Attributes$src(data.fileName),
										A2(
										$elm$html$Html$Attributes$property,
										'frameborder',
					...
					default:
						...
						return A2(
							...
							A2(
								...
								function (_v1) {
									var href = _v1.href;
									var textDescription = _v1.textDescription;
									return A2(
										...
										{
											label: $mdgriffith$elm_ui$Element$text(textDescription),
											url: href
										});
								},
								urls));
...
var $author$project$Project$mediumToString = function (medium) {
	switch (medium.$) {
		case 'Installation':
			return 'installation';
		case 'Game':
...
var $author$project$Viewers$ProjectsViewer$CenterView$CenterView$mediumLabel = function (tags) {
	return A2(
		$mdgriffith$elm_ui$Element$el,
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Element$Font$size(15),
				$mdgriffith$elm_ui$Element$Background$color(
				A3($mdgriffith$elm_ui$Element$rgb255, 254, 80, 0)),
				$mdgriffith$elm_ui$Element$Font$color(
		...
		$mdgriffith$elm_ui$Element$text(
			$author$project$Project$mediumToString(tags.medium)));
};
...
var $author$project$Viewers$ProjectsViewer$CenterView$CenterView$projectTransitionAnimation = F4(
	function (transition, openProject, from, to) {
		return A4(
			$author$project$Viewers$ProjectsViewer$CenterView$CenterView$transitionAnimation,
			transition,
			function (currProject) {
				return _Utils_eq(currProject.id, openProject.id);
			},
			from,
			to);
	});
...
var $author$project$Project$toolToString = function (tool) {
	switch (tool.$) {
		case 'Touchdesigner':
...
var $author$project$Viewers$ProjectsViewer$CenterView$CenterView$toolstackDisplay = function (tags) {
	return A2(
		$elm$core$List$map,
		function (tool) {
			return A2(
				...
				$mdgriffith$elm_ui$Element$text(
					$author$project$Project$toolToString(tool)));
		},
		tags.toolStack);
};
var $author$project$Types$NewFootageTypeClicked = function (a) {
	return {$: 'NewFootageTypeClicked', a: a};
};
var $author$project$Project$Process = {$: 'Process'};
var $author$project$Viewers$ProjectsViewer$CenterView$CenterView$typeOfFootageSelector = function (currTypeOfImage) {
	return A2(
		$elm$core$List$map,
		function (imageTypeButton) {
			return A2(
				$mdgriffith$elm_ui$Element$Input$button,
				_List_fromArray(
					[
						...
						A3(
							$author$project$Funcs$when,
							_Utils_eq(currTypeOfImage, imageTypeButton),
							A3($mdgriffith$elm_ui$Element$rgb, 0.5, 0.5, 0.5),
							A3($mdgriffith$elm_ui$Element$rgb, 0.3, 0.3, 0.3)))
					]),
				{
					label: $mdgriffith$elm_ui$Element$text(
						_Utils_eq(imageTypeButton, $author$project$Project$Final) ? 'final' : (_Utils_eq(imageTypeButton, $author$project$Project$Process) ? 'process' : '')),
					onPress: $elm$core$Maybe$Just(
						$author$project$Types$NewFootageTypeClicked(imageTypeButton))
				});
		},
		_List_fromArray(
			[$author$project$Project$Final, $author$project$Project$Process]));
};
var $author$project$Project$unpackFootage = function (footage) {
	switch (footage.$) {
		case 'Image':
			var data = footage.a;
...
var $author$project$Project$clientToString = function (client) {
	switch (client.$) {
		case 'Azerion':
			return 'Azerion';
		case 'HKU':
...
var $author$project$Viewers$ProjectsViewer$CenterView$CenterView$variablesDisplay = F2(
	function (variables, sources) {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
				[
							...
							'Date: ' + A2(
								...
								A3(
									...
									A2(
										...
										'-',
										$justinmimbs$date$Date$toIsoString(variables.date))))))
					]),
				((!_Utils_eq(variables.client, $author$project$Project$None)) && (!_Utils_eq(variables.clientLink, $elm$core$Maybe$Nothing))) ? _List_fromArray(
					[
						A2(
						$mdgriffith$elm_ui$Element$newTabLink,
						_List_fromArray(
						...
						{
							label: $mdgriffith$elm_ui$Element$text(
								'Together with ' + $author$project$Project$clientToString(variables.client)),
							url: A2($elm$core$Maybe$withDefault, '', variables.clientLink)
						})
					]) : ((!_Utils_eq(sources.externalLink, $elm$core$Maybe$Nothing)) ? _List_fromArray(
					[
						A2(
						$mdgriffith$elm_ui$Element$newTabLink,
						_List_fromArray(
							...
							]),
						{
							...
							url: A2($elm$core$Maybe$withDefault, '', sources.externalLink)
						})
					]) : _List_Nil)));
	});
var $author$project$Viewers$ProjectsViewer$CenterView$CenterView$centerViewer = F6(
	function (importStyles, projectTransition, footageTransition, footageType, muted, isPortrait) {
		var project = $mdgriffith$elm_animator$Animator$current(projectTransition);
		var processFootageExists = function () {
			var amountOfProcessFootage = $elm$core$List$length(project.sources.processFootage);
			return amountOfProcessFootage > 0;
		}();
		var footageIndex = $mdgriffith$elm_animator$Animator$current(footageTransition);
		var currFootageAbout = $mdgriffith$elm_animator$Animator$current(footageType);
		var footageToShow = A2($author$project$Project$getAppropriateFootage, currFootageAbout, project);
		var amountOfFootageInProject = $elm$core$List$length(footageToShow);
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_Utils_ap(
				importStyles,
				_List_fromArray(
					[
						...
						A3(
							$elm$core$List$foldl,
							F2(
								function (footage, foundAVideo) {
									return foundAVideo || function () {
										if (footage.$ === 'Video') {
											return true;
										} else {
											return false;
							...
							false,
							A2($author$project$Project$getAppropriateFootage, currFootageAbout, project)) ? A2(
							$mdgriffith$elm_ui$Element$Input$button,
							_List_fromArray(
								[
								...
								]),
							{
								label: $mdgriffith$elm_ui$Element$text(
									muted ? '🔇' : '🔉'),
								onPress: $elm$core$Maybe$Just($author$project$Types$ToggleMute)
							}) : $mdgriffith$elm_ui$Element$none)
					])),
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									function () {
									var titleLength = $elm$core$String$length(project.text.name);
									var titleAnimation = A4($author$project$Viewers$ProjectsViewer$CenterView$CenterView$projectTransitionAnimation, projectTransition, project, 1, titleLength);
									return A2(
										$mdgriffith$elm_ui$Element$el,
										_List_fromArray(
											[
										...
										$mdgriffith$elm_ui$Element$text(
											A2(
												$elm$core$String$left,
												$elm$core$Basics$round(titleAnimation),
												project.text.name)));
								}(),
									A2(
									$mdgriffith$elm_ui$Element$row,
									...
									$author$project$Viewers$ProjectsViewer$CenterView$CenterView$toolstackDisplay(project.tags))
								])),
							A2(
							$mdgriffith$elm_ui$Element$column,
							_List_fromArray(
							...
							_List_fromArray(
								[
									$author$project$Viewers$ProjectsViewer$CenterView$CenterView$mediumLabel(project.tags),
									A2($author$project$Viewers$ProjectsViewer$CenterView$CenterView$variablesDisplay, project.variables, project.sources)
								]))
						])),
					processFootageExists ? A2(
					$mdgriffith$elm_ui$Element$row,
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
					...
					$author$project$Viewers$ProjectsViewer$CenterView$CenterView$typeOfFootageSelector(currFootageAbout)) : $mdgriffith$elm_ui$Element$none,
					A2(
					$mdgriffith$elm_ui$Element$row,
					_List_fromArray(
						[
					...
					_List_fromArray(
						[
							A5($author$project$Viewers$ProjectsViewer$CenterView$CenterView$footageNavigationButtons, $author$project$Types$Left, !isPortrait, footageIndex > 0, footageTransition, '<'),
							A4(
							$author$project$Viewers$ProjectsViewer$CenterView$FootageViewer$footageView,
							projectTransition,
							footageTransition,
							muted,
							A2($elm_community$list_extra$List$Extra$getAt, footageIndex, footageToShow)),
							A5(
							$author$project$Viewers$ProjectsViewer$CenterView$CenterView$footageNavigationButtons,
							$author$project$Types$Right,
							!isPortrait,
							_Utils_cmp(footageIndex, amountOfFootageInProject - 1) < 0,
							footageTransition,
							'>')
						])),
					A2(
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$centerX,
							$mdgriffith$elm_ui$Element$alpha(
							A3($author$project$Funcs$when, amountOfFootageInProject > 1, 0.8, 0)),
							$mdgriffith$elm_ui$Element$spacing(30)
						]),
					function () {
						var footageSelectorDot = F2(
							function (associatedFootageIndex, character) {
								return A2(
									$mdgriffith$elm_ui$Element$el,
									_List_fromArray(
										[
											...
											$author$project$Types$FootageIndexClicked(associatedFootageIndex)),
											...
											$elm$core$Basics$round(
												A4(
													...
													footageTransition,
													function (state) {
														return _Utils_eq(state, associatedFootageIndex);
													},
													20,
													30))),
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Font$size(
							A3($author$project$Funcs$when, isPortrait, 20, 10)),
							$mdgriffith$elm_ui$Element$centerX,
							$mdgriffith$elm_ui$Element$Font$center,
							$mdgriffith$elm_ui$Element$padding(10)
						]),
									...
									A2(
										...
										A2(
											...
											function (footage) {
												return A2(
													$elm$core$Maybe$andThen,
													function (footageMetadata) {
														return $elm$core$Maybe$Just(footageMetadata.description);
													},
													$author$project$Project$unpackFootage(footage));
											},
											A2($elm_community$list_extra$List$Extra$getAt, footageIndex, footageToShow))))
								]))
						]))
...
var $author$project$Viewers$ProjectsViewer$Description$Left = {$: 'Left'};
var $author$project$Viewers$ProjectsViewer$Description$Right = {$: 'Right'};
...
var $author$project$Viewers$ProjectsViewer$Description$description = F4(
	function (importStyles, projectTransition, isPortrait, viewerPart) {
		var quiteAttributes = _Utils_ap(
			A3(
				$author$project$Funcs$when,
				isPortrait,
				_List_fromArray(
		...
		var project = $mdgriffith$elm_animator$Animator$current(projectTransition);
		var alignToFontAlign = function (align) {
			return A3(
				$author$project$Funcs$when,
				_Utils_eq(align, $author$project$Viewers$ProjectsViewer$Description$Left),
				$mdgriffith$elm_ui$Element$Font$alignLeft,
				$mdgriffith$elm_ui$Element$Font$alignRight);
		};
		var paragraphAttributes = function (align) {
			return _Utils_ap(
				A3(
					$author$project$Funcs$when,
					isPortrait,
		...
		var subtitleAttributes = function (align) {
			return _Utils_ap(
				A3(
					$author$project$Funcs$when,
					isPortrait,
		...
		var subtitleParagraphPair = F3(
			function (align, titleText, paragraphText) {
				return (paragraphText === '') ? _List_fromArray(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$mdgriffith$elm_ui$Element$text(paragraphText)
									]))
							]))
					]);
			});
				...
				[
					...
					A2(
						...
						A2(
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$text(project.text.shortDescription)
								])),
						_Utils_ap(
							A3(subtitleParagraphPair, $author$project$Viewers$ProjectsViewer$Description$Left, 'Context', project.text.context),
							_Utils_ap(
								A3(subtitleParagraphPair, $author$project$Viewers$ProjectsViewer$Description$Right, 'Description', project.text.longDescription),
								_Utils_ap(
									A3(subtitleParagraphPair, $author$project$Viewers$ProjectsViewer$Description$Left, 'Process', project.text.processDescription),
									_Utils_ap(
										A3(subtitleParagraphPair, $author$project$Viewers$ProjectsViewer$Description$Right, 'Philosophy behind it', project.text.philosophy),
										A3(subtitleParagraphPair, $author$project$Viewers$ProjectsViewer$Description$Left, 'Reflections', project.text.reflection)))))))
				]));
	});
var $author$project$Viewers$ProjectsViewer$ProjectsViewer$hoverAnimation = F4(
	function (model, viewPart, from, to) {
		return A2(
			$mdgriffith$elm_animator$Animator$move,
			model.hovered,
...
var $author$project$Viewers$ProjectsViewer$ProjectPicker$projectButton = F3(
	function (model, toProjectTransition, project) {
		var url = model.url;
		return A2(
			$mdgriffith$elm_ui$Element$row,
			_List_fromArray(
				[
					...
					A2(
						...
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$alpha(
								A3(toProjectTransition, project.id, 0, 1)),
								$mdgriffith$elm_ui$Element$Font$color(
								A3($mdgriffith$elm_ui$Element$rgb, 0.8, 0.8, 0.8)),
								$mdgriffith$elm_ui$Element$Background$color(
								A3($mdgriffith$elm_ui$Element$rgb, 0.3, 0.3, 0.3)),
								...
								$elm$core$Basics$round(
									A3(toProjectTransition, project.id, 0, 20)),
								20),
								$mdgriffith$elm_ui$Element$centerY,
						...
						$mdgriffith$elm_ui$Element$text(
							A3($author$project$Funcs$when, model.onMobile, '^', '<'))))
				]),
			_List_fromArray(
				[
					A2(
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$mdgriffith$elm_ui$Element$alpha(
											A3(toProjectTransition, project.id, 0.8, 1))
										])),
									$mdgriffith$elm_ui$Element$scale(
									A3(toProjectTransition, project.id, 1, 0.92)),
									$mdgriffith$elm_ui$Element$width(
								...
								[
									...
									_List_fromArray(
										[
											...
											A2(
												$mdgriffith$elm_ui$Element$maximum,
												A3($author$project$Funcs$when, model.onMobile, 400, 228),
									...
									_List_fromArray(
										[
											...
											_List_fromArray(
												[
													$mdgriffith$elm_ui$Element$Font$size(
													A3($author$project$Funcs$when, model.onMobile, 30, 18)),
													$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
													$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$fill),
													$mdgriffith$elm_ui$Element$Font$alignRight,
													$mdgriffith$elm_ui$Element$padding(5),
											...
											_List_fromArray(
												[
													$mdgriffith$elm_ui$Element$text(project.text.name)
												])),
											A2(
											...
											_List_fromArray(
												[
													...
													A3($mdgriffith$elm_ui$Element$rgb, 0.1, 0.1, 0.1)),
													$mdgriffith$elm_ui$Element$htmlAttribute(
													$elm$html$Html$Attributes$draggable('flase')),
													$mdgriffith$elm_ui$Element$clip,
													$mdgriffith$elm_ui$Element$centerX
												]),
											{
												description: project.sources.thumbnail.description,
												src: A2($author$project$Project$getFootagePath, project.sources.thumbnail, project)
											})
										]))
								])),
						onPress: $elm$core$Maybe$Just(
							$author$project$Types$LinkClicked(
								$elm$browser$Browser$Internal(
									_Utils_update(
										url,
										{
											fragment: $elm$core$Maybe$Just(project.id)
										}))))
					})
				]));
	});
var $author$project$Funcs$styleWhen = F2(
	function (condition, trueValue) {
		return condition ? trueValue : _List_Nil;
	});
var $author$project$Viewers$ProjectsViewer$ProjectPicker$projectPicker = F2(
	function (styles, model) {
		var toProjectTransition = F3(
			function (id, off, on) {
				return A2(
					$mdgriffith$elm_animator$Animator$move,
					model.projectTransition,
					function (proj) {
						return A3(
							$author$project$Funcs$when,
							_Utils_eq(proj.id, id),
							A2(
								$mdgriffith$elm_animator$Animator$leaveLate,
								0.5,
								$mdgriffith$elm_animator$Animator$at(on)),
							$mdgriffith$elm_animator$Animator$at(off));
					});
			});
		return A2(
			$mdgriffith$elm_ui$Element$column,
			...
			_Utils_ap(
				A2(
					$author$project$Funcs$styleWhen,
					!model.onMobile,
					_List_fromArray(
						[
							A2(
							...
							_List_fromArray(
								[
									...
									$mdgriffith$elm_ui$Element$padding(20),
									$mdgriffith$elm_ui$Element$Background$color(
									A3($mdgriffith$elm_ui$Element$rgb, 0.5, 0.7, 0.5))
								]),
							{
								label: $mdgriffith$elm_ui$Element$text('running experimentations'),
								url: 'https://www.instagram.com/coelepinda/'
							})
						])),
				A2(
					$elm$core$List$map,
					function (project) {
						return A3($author$project$Viewers$ProjectsViewer$ProjectPicker$projectButton, model, toProjectTransition, project);
					},
					$author$project$Projects$AllProjects$projects)));
	});
...
var $author$project$Viewers$ProjectsViewer$ProjectsViewer$projectViewer = function (model) {
	var partTitle = F2(
			...
			return A2(
				...
				_List_fromArray(
					[
						...
						A3(
						$author$project$Funcs$when,
						_Utils_eq(part, $author$project$Types$ProjectPicker),
						$mdgriffith$elm_ui$Element$alignRight,
						$mdgriffith$elm_ui$Element$alignLeft),
						A3(
						$author$project$Funcs$when,
						...
						A3($mdgriffith$elm_ui$Element$rgb, 0.9, 0.9, 0.9)),
						$mdgriffith$elm_ui$Element$alpha(
						A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation, model, part, 0, 1))
					]),
				$mdgriffith$elm_ui$Element$text(partName));
		});
	var partSize = function (part) {
		return (!model.onMobile) ? _List_fromArray(
			[
				$mdgriffith$elm_ui$Element$width(
				A3(
					$elm$core$Basics$apL,
					$mdgriffith$elm_ui$Element$maximum,
					$elm$core$Basics$round(
						A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation, model, part, 80, 400)),
			...
			[
				...
				A3(
					...
					$mdgriffith$elm_ui$Element$maximum,
					$elm$core$Basics$round(
						A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation, model, part, 80, model.screenSize.h)),
					$mdgriffith$elm_ui$Element$fill))
			]);
	};
	var partColor = function (part) {
		return $mdgriffith$elm_ui$Element$Background$color(
			A4(
				$mdgriffith$elm_ui$Element$rgba,
				0.3,
				0.3,
				A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation, model, part, 0.7, 0.5),
				1));
	};
	var partBanner = F2(
		function (partName, part) {
				...
				_Utils_ap(
					_List_fromArray(
						[
							...
							$mdgriffith$elm_ui$Element$centerX,
							$mdgriffith$elm_ui$Element$alpha(
							A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation, model, part, 1, 0))
						]),
					A3(
						$author$project$Funcs$when,
						model.onMobile,
						...
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$rotate($elm$core$Basics$pi * 0.5),
								$mdgriffith$elm_ui$Element$Font$size(30)
							]))),
				$mdgriffith$elm_ui$Element$text(partName));
		});
	var flowDirection = A3($author$project$Funcs$when, model.onMobile, $mdgriffith$elm_ui$Element$column, $mdgriffith$elm_ui$Element$row);
	return A2(
		flowDirection,
		_List_fromArray(
			[
			...
			[
				...
				_Utils_ap(
					_Utils_ap(
						...
						partSize($author$project$Types$Description)),
					_Utils_ap(
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$alignLeft,
								partColor($author$project$Types$Description),
								$mdgriffith$elm_ui$Element$htmlAttribute(
								A2(
									$elm$html$Html$Attributes$style,
									'overflow',
									(!_Utils_eq(
										$mdgriffith$elm_animator$Animator$arrived(model.activeViewerPart),
										$author$project$Types$Description)) ? 'clip' : 'scroll')),
								$mdgriffith$elm_ui$Element$htmlAttribute(
								A2($elm$html$Html$Attributes$style, 'height', '100vh')),
								A3(
								$author$project$Funcs$when,
								_Utils_eq(
									$mdgriffith$elm_animator$Animator$current(model.activeViewerPart),
									$author$project$Types$ProjectPicker),
								$mdgriffith$elm_ui$Element$pointer,
								$mdgriffith$elm_ui$Element$alpha(1)),
								$mdgriffith$elm_ui$Element$inFront(
								A2(partBanner, 'DESCRIPTION', $author$project$Types$Description)),
								$mdgriffith$elm_ui$Element$Events$onMouseEnter(
								$author$project$Types$NewHover($author$project$Types$Description)),
								$mdgriffith$elm_ui$Element$Events$onMouseLeave(
								$author$project$Types$NewHover($author$project$Types$Background)),
								$mdgriffith$elm_ui$Element$Events$onClick(
								$author$project$Types$NewPagePartActive($author$project$Types$Description))
							]),
						A2(
							$author$project$Funcs$styleWhen,
							_Utils_eq(
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$alpha(
									A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$hoverAnimation, model, $author$project$Types$Description, 1, 0.7)),
									$mdgriffith$elm_ui$Element$moveLeft(
									A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$hoverAnimation, model, $author$project$Types$Description, 0, 3))
								])))),
				_List_fromArray(
					[
						A2(partTitle, 'DESCRIPTION', $author$project$Types$Description),
						A4(
						$author$project$Viewers$ProjectsViewer$Description$description,
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$fill),
								$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
								$mdgriffith$elm_ui$Element$alpha(
								A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation, model, $author$project$Types$Description, 0, 1))
							]),
						model.projectTransition,
						model.onMobile,
						model.activeViewerPart)
					...
					A2(
						...
						_List_fromArray(
							[
								...
								$mdgriffith$elm_ui$Element$paddingXY,
								$elm$core$Basics$round(
									A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation, model, $author$project$Types$Background, 25, 200)),
								25)
							]))),
				_List_fromArray(
					[
						A6(
						$author$project$Viewers$ProjectsViewer$CenterView$CenterView$centerViewer,
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
								$mdgriffith$elm_ui$Element$centerY,
				...
				_Utils_ap(
					_Utils_ap(
						...
						partSize($author$project$Types$ProjectPicker)),
					_Utils_ap(
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$alignRight,
								partColor($author$project$Types$ProjectPicker),
								$mdgriffith$elm_ui$Element$htmlAttribute(
								A2(
									$elm$html$Html$Attributes$style,
									'overflow',
									(!_Utils_eq(
										$mdgriffith$elm_animator$Animator$arrived(model.activeViewerPart),
										$author$project$Types$ProjectPicker)) ? 'clip' : 'scroll')),
								$mdgriffith$elm_ui$Element$htmlAttribute(
								A2($elm$html$Html$Attributes$style, 'height', '100vh')),
								$mdgriffith$elm_ui$Element$inFront(
								A2(partBanner, 'PROJECT PICKER', $author$project$Types$ProjectPicker)),
								$mdgriffith$elm_ui$Element$Events$onClick(
								$author$project$Types$NewPagePartActive($author$project$Types$ProjectPicker)),
								$mdgriffith$elm_ui$Element$Events$onMouseEnter(
								$author$project$Types$NewHover($author$project$Types$ProjectPicker)),
								$mdgriffith$elm_ui$Element$Events$onMouseLeave(
								$author$project$Types$NewHover($author$project$Types$Background)),
								A3(
								$author$project$Funcs$when,
								_Utils_eq(
									$mdgriffith$elm_animator$Animator$current(model.activeViewerPart),
									$author$project$Types$Description),
								$mdgriffith$elm_ui$Element$pointer,
								$mdgriffith$elm_ui$Element$alpha(1))
							]),
						A2(
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$alpha(
									A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$hoverAnimation, model, $author$project$Types$ProjectPicker, 1, 0.7)),
									$mdgriffith$elm_ui$Element$moveRight(
									A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$hoverAnimation, model, $author$project$Types$ProjectPicker, 0, 3))
								])))),
				_List_fromArray(
					[
						A2(partTitle, 'PROJECT PICKER', $author$project$Types$ProjectPicker),
						A2(
						$author$project$Viewers$ProjectsViewer$ProjectPicker$projectPicker,
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
								$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$fill),
								$mdgriffith$elm_ui$Element$alpha(
								A4($author$project$Viewers$ProjectsViewer$ProjectsViewer$activeViewPartAnimation, model, $author$project$Types$ProjectPicker, 0, 1))
							]),
						model)
					]))
			]));
		...
		body: _List_fromArray(
			[
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$inFront(
						$author$project$Effects$LoadAnimation$loadFadein(model.loaded)),
						$mdgriffith$elm_ui$Element$Background$color(
						A3($mdgriffith$elm_ui$Element$rgb, 0.2, 0.2, 0.2)),
						...
						_List_fromArray(
							[
								...
								{name: 'Trispace', url: 'https://fonts.googleapis.com/css2?family=Trispace:wght@400&display=swap'}),
				...
				A2(
					...
					$author$project$Viewers$ProjectsViewer$ProjectsViewer$projectViewer(model)))
			]),
		title: 'David Zwitser'
	};
};
var $author$project$Main$main = $elm$browser$Browser$application(
	{
		...
		subscriptions: function (model) {
			return $elm$core$Platform$Sub$batch(
				_List_fromArray(
					[
						A3($mdgriffith$elm_animator$Animator$toSubscription, $author$project$Types$TimeTick, model, $author$project$Main$animator),
						$elm$browser$Browser$Events$onResize(
						F2(
							function (w, h) {
								return A2($author$project$Types$GotNewScreenSize, w, h);
							}))
					]));
		},
